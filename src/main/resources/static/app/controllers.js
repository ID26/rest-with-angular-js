(function (angular) {
    var RandomNumberController = function ($scope, RandomNumber) {
        ...
        $scope.addRandomNumber = function (nickname, numberFrom, numberTo) {
            ...
            alert("Ты счастливчик " + value);
            new RandomNumber({
                nickname: $scope.validation(nickname) ? nickname : "Введите никнейм",
                value: value,
                createDate: new Date()
            }).$add(function (result) {
        ...
        angular.module("rest-with-angular-js.controllers").controller("RandomNumberController", RandomNumberController);
